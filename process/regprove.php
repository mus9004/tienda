...
$corProve= $_POST['prove-cor'];

if(!$nitProve=="" && !$nameProve=="" && !$dirProve=="" && !$telProve=="" && !$webProve=="" && !$corProve==""){
    $verificar=  ejecutarSQL::consultar("select * from proveedor where NITProveedor='".$nitProve."'");
    $verificaltotal = mysql_num_rows($verificar);
    if($verificaltotal<=0){
        if(consultasSQL::InsertSQL("proveedor", "NITProveedor, NombreProveedor, Direccion, Telefono, PaginaWeb, Correo", "'$nitProve','$nameProve','$dirProve','$telProve','$webProve','$corProve'")){
        ...
        echo '<img src="assets/img/incorrecto.png" class="center-all-contens"><br><p class="lead text-center">El número de RTN que ha ingresado ya existe.<br>Por favor ingrese otro número de NIT</p>';
    }
}else {
